...
fun UpdateNoteScreenRoot(
    ...
    LaunchedEffect(state.navigateBack) {
        if (state.navigateBack) {
            navigateBack()
            viewModel.onAction(UpdateNoteAction.ResetNavigation)
        }
    }
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(onClick = { action(UpdateNoteAction.NavigateBack) }) {
                        Icon(
        ...
        LazyColumn(
            ...
            item {
                NoteTextField(
                    value = state.title,
                    onValueChange = { action(UpdateNoteAction.TitleChange(it)) },
                ...
                NoteTextField(
                    value = state.content,
                    onValueChange = { action(UpdateNoteAction.ContentChange(it)) },
